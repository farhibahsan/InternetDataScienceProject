...
class CensysDataset(Dataset):
    def __init__(self, filepath, class_list=None):
        ...
        if self.class_list is None:
            ...
            self.service_set = set(class_list)
        self.vendor_set = set()
    ...
    def __getitem__(self, idx):
        ...
        country = self.cc_le.transform([country])[0]
        ...
        country_asn = self.cc_asn_le.transform([country_asn])[0]

        # Do the same for vendor, and product
        vendor = self.vendor_list[idx]
        vendor = self.vendor_le.transform([vendor])[0]

        product = self.product_list[idx]
        product = self.product_le.transform([product])[0]
        ...
        if self.class_list is None:
            ...
            intersection = [service for service in services if service in self.class_list]
            services = self.service_le.transform(intersection)
